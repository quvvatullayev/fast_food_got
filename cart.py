...
class Cart:
    ...
    def add_plus(self, update: Update, context: CallbackContext):
        query = update.callback_query
        bot = context.bot
        chat_id = query.message.chat_id
        product_id = int(query.data.split("_")[-1])
        n = db.add_plus(chat_id, product_id)
        reply_markup = [
            [
                InlineKeyboardButton('-', callback_data=f"minus_{product_id}"),
                InlineKeyboardButton(f'{n}', callback_data=f"n"),
    ...
    def add_minus(self, update: Update, context: CallbackContext):
        query = update.callback_query
        bot = context.bot
        chat_id = query.message.chat_id
        product_id = int(query.data.split("_")[-1])
        n = db.add_minus(chat_id, product_id)
        reply_markup = [
            [
                InlineKeyboardButton('-', callback_data=f"minus_{product_id}"),
                InlineKeyboardButton(f'{n}', callback_data=f"n"),
